...
public:
	...
	inline void operator delete(void* point, std::align_val_t allgin_val, void* block)
	{
		MemoryPool::GetInstance()->Deallocate(point);
	}
...
{
	return std::unique_ptr<T, std::function<void(T*)>>(New<T>(std::forward<Args>(args)...), Delete<T>);
}
