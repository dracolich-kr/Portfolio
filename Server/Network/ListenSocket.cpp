...
namespace Network
{
	ErrorResult ListenSocket::Open()
	{
		...
		{
			LOG_DEBUG_FMT(mLogger, L"WSAIoctl failed with error: %u\n", WSAGetLastError());
		}
	...
	ErrorResult ListenSocket::Bind(Port port)
	{
		...
		{
			LOG_DEBUG(mLogger, "Error - Fail bind \n");
			Close();
	...
	ErrorResult ListenSocket::Listen(Int32 back_log)
	{
		...
		{
			LOG_DEBUG(mLogger, "Error - Fail listen \n");
			// 6. 소켓종료
			Close();
			return ErrorResult::SocketFailed;
		}
	...
	ErrorResult ListenSocket::Accept(std::shared_ptr<Socket> client, IocpAcceptOverlapped* overlapped)
	{
		ErrorResult result = ErrorResult::Success;
		...
		overlapped->IocpHandle = mIocp->GetHandle();
		overlapped->ListenSocket = shared_from_this();
